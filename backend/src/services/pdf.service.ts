...
class PDFService {
  ...
  constructor() {
    this.outputDir = path.resolve(env.upload.uploadDir, 'invoices');
    if (!fs.existsSync(this.outputDir)) {
      fs.mkdirSync(this.outputDir, { recursive: true });
    }
  }

  async generateInvoice(data: InvoiceData): Promise<{ filePath: string; filename: string }> {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (data.project_name) doc.text(`Project: ${data.project_name}`);
      doc.moveDown();
      ...
      data.items.forEach((item) => {
        ...
        x += colWidths[1];
        doc.text(formatCurrency(item.unit_price), x, y, { width: colWidths[2] });
        x += colWidths[2];
        doc.text(formatCurrency(item.total_price), x, y, { width: colWidths[3] });
        y += 20;
      });

      doc.moveDown(2);
      y = doc.y;
      doc.text(`Subtotal: ${formatCurrency(data.subtotal)}`, 350, y, { align: 'right', width: 195 });
      doc.text(`GST (${data.gst_rate}%): ${formatCurrency(data.gst_amount)}`, 350, y + 15, {
        align: 'right',
        width: 195,
      });
      doc.font('Helvetica-Bold');
      doc.text(`Total: ${formatCurrency(data.total_amount)}`, 350, y + 30, {
        align: 'right',
        width: 195,
      });
      ...
      doc.fontSize(8).text(
        'This is a computer-generated invoice. BuildMaster ERP - All rights reserved.',
        50,
        750,
  ...
  async generateQuotation(data: {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (data.valid_until) doc.text(`Valid Until: ${data.valid_until}`);
      doc.moveDown();
      ...
      lines.forEach(([label, amount]) => {
        doc.text(`${label}: ${formatCurrency(amount as number)}`);
      });

      doc.moveDown();
      doc.font('Helvetica-Bold').text(`Total: ${formatCurrency(data.total_amount)}`);
      doc.end();
