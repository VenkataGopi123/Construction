...
class AIService {
    ...
    async callGemini(prompt) {
        if (!env_1.env.gemini.apiKey)
            return null;
        try {
            const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-pro:generateContent?key=${env_1.env.gemini.apiKey}`, {
                method: 'POST',
            ...
            try {
                const completion = await openai.chat.completions.create({
                    ...
                    messages: [
                        {
                            role: 'system',
                            content: 'You are a construction industry expert for BuildMaster ERP. Provide concise, actionable JSON-friendly analysis for Indian construction projects.',
                        },
